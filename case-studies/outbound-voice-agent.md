...
| 일 평균 통화 | 2,000건/일 |
| 도입 병원 수 | 100개+ |
| 응답 레이턴시 | ~300ms (기존 ~900ms 대비 67% 감소) |
...
기존 파이프라인 방식과 Realtime API의 레이턴시 차이입니다.
...
일 2,000건의 통화에서 각각 무슨 일이 있었는지 파악해야 합니다. 예약이 완료되었는지, 상담원 연결이 필요한지, 환자가 긍정적으로 반응했는지 등을 자동으로 분류해야 합니다.
...
| OpenAI Realtime API | STT→LLM→TTS 레이턴시 ~900ms | 단일 Realtime 호출 | ~300ms (67% 감소) |
